...
if (!Object.getOwnPropertyDescriptor(Module["ready"], "_enterjsGame")) {
 Object.defineProperty(Module["ready"], "_enterjsGame", {
  configurable: true,
  get: function() {
   abort("You are getting _enterjsGame on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js");
  }
 });
 Object.defineProperty(Module["ready"], "_enterjsGame", {
  configurable: true,
  set: function() {
   abort("You are setting _enterjsGame on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js");
  }
 });
}
...
assert(INITIAL_MEMORY == 79101952, "Detected runtime INITIAL_MEMORY setting.  Use -sIMPORTED_MEMORY to define wasmMemory dynamically");
...
if (Module["locateFile"]) {
 ...
 wasmBinaryFile = new URL("calc.wasm", import.meta.url).toString();
}
...
function readAsmConstArgs(sigPtr, buf) {
 ...
 while (ch = _asan_js_load_1u(sigPtr++)) {
  assert(ch === 100 || ch === 102 || ch === 105 || ch === 106, "Invalid character " + ch + '("' + String.fromCharCode(ch) + '") in readAsmConstArgs! Use only "d", "f" or "i", and do not specify "v" for void return argument.');
  assert(ch !== 106, "i64 arguments to ASM_JS function are not available without WASM_BIGINT");
  buf += ch != 105 & buf;
...
var _enterjsGame = Module["_enterjsGame"] = createExportWrapper("enterjsGame");
